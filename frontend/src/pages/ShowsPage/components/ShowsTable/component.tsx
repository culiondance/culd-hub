  ...
  const columns = [
      ...
      render: (priority: number, show: Show) => {
        return (
          ...
          >
            {(!show.isOpen
              ? "closed"
              : (showPriorityChoices[priority] ?? priority)
            ).toUpperCase()}
          </Tag>
        );
      },
            ...
            <Space size={5}>
              ...
              {show.isCampus && (
                <Tooltip
                  ...
                  style={{ textAlign: "center" }}
                >
                  <CrownFilled style={{ color: "#4096ff" }} />
                </Tooltip>
              )}
              {show.isOutOfCity && (
                <Tooltip
    ...
    },
    {
      title: (
        <span>
          {"Tentative Roster"}
          <Tooltip title="Final confirmations via Messenger" placement="right">
            <InfoCircleTwoTone
      ...
      render: (performers: Member[], show: Show) => (
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          <Space wrap>
            {performers
              ...
              .sort((a: Member, b: Member) => {
                return a.user.id === show.point?.user.id
                  ? -1
                  : b.user.id === show.point?.user.id
                    ? 1
              ...
              .map((performer: Member) => (
                ...
                >
                  <Tag
                    ...
                    color={
                      performer.user.id === show.point?.user.id
                        ? "volcano"
                        : null
                    }
                  >
          ...
          <Tooltip
            title={
              show.lions
                ? `${performers.length} of ${performersNeeded(show)} performers`
            ...
            <Progress
              ...
              percent={
                show.lions == null
                  ? 0
                  : Math.round(
                      (performers.length / performersNeeded(show)) * 100,
                    )
              }
