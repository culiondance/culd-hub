...
const ShowDetails = ({ show }: { show: Show }) => {
  return (
    <Descriptions layout="vertical" bordered size="middle">
      ...
      <Descriptions.Item label="Lions">{show.lions || ""}</Descriptions.Item>
      ...
      >
        <Space wrap>
          {show.performers.slice().map((performer: Member) => (
            <Tag
      ...
      <Descriptions.Item label="Notes">{show.notes || ""}</Descriptions.Item>
    </Descriptions>
  );
};
